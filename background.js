...
P（計画）
- 次回経過観察とする
- 定期処方の薬についてはAで立てた問いに対して、問題なさそうという柔らかい言い回しで記載する
  （例：Aで「カロナールの使用頻度はどうか？」→ Pでは「カロナールの使用頻度は問題なさそう。」）
  （例：Aで「オキシブチニンによる便秘への影響はどうか？」→ Pでは「オキシブチニンによる便秘への影響は問題なさそう。」）
- 「問題なかった」「問題ない」ではなく「問題なさそう」を使う
- 初回投薬・追加処方の薬については「問題なさそう」とは書かず、次回確認事項として記載する
...
P - Aの問いに対して「問題なさそう」の形式で応答する（例：「亜鉛補充による成長や皮膚症状の改善傾向は問題なさそう。」）。次回も血清亜鉛値の確認を行う旨を記載する
...
13. 便秘薬が処方されているのにS/O/Pで「便通は問題ない」「便通の状況は問題なかった」と記載していないか確認する。便秘薬が出ている以上、便通に何らかの問題がある前提で記載する
14. 酢酸亜鉛が処方されている場合、患者が乳幼児であれば「味覚障害」「味覚異常」を処方理由として記載していないか確認する（乳幼児は味覚異常を訴えられないため不適切）。乳幼児以外であれば味覚障害の記載は問題ない
15. 不整合があれば修正してから出力すること
